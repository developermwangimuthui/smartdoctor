                ...
                <div class="box_general_3 booking">
                    ...
                    @endif 
                        ...
                        <div class="row" style="margin-top: -24px;">
                            ...
                            <div class="col-md-6">
                            <h6 class="text-success">Date: {{Helper::globalDate(date("Y-m-d"))}}</h6>
                                <table class="table table-warning"  id="table_schedule1">
                                    ...
                                    @foreach (\App\DoctorAvailable::where('status',null)->where('doctor_id',$doctor->id)->where('date',date("Y-m-d"))->get() as $item)
                                    <tr>
                            ...
                            <div class="col-md-6">
                            <h6 class="text-success">Date: {{Helper::globalDate(date("Y-m-d", strtotime("1 days")))}}  </h6>
                                <table class="table table-warning"  id="table_schedule2">
                                    <tr  class="bg-success text-white">
                                        <th>Start</th>
                                        <th>End</th>
                                        <th>Book</th>
                                    </tr>
                                    @foreach (\App\DoctorAvailable::where('status',null)->where('doctor_id',$doctor->id)->where('date',date("Y-m-d", strtotime("1 days")))->get() as $item)
                                    <tr> 
                                        <td>{{$item->starting_time}}</td>
                                        <td>{{$item->ending_time}}</td>
                                        <td><a class="btn btn-warning btn-sm" onclick="alertShow(this.id,'{{$item->date}}','{{$item->starting_time}}','{{$item->ending_time}}')" id="{{ url('/charge?starting_time='.$item->starting_time.'&ending_time='.$item->ending_time.'&date='.$item->date.'&doctor_id='.$item->doctor_id.'&id='.$item->id.'&sadasdas='.$charge.'&booking='.$booking.'&service='.$service) }}" href="#">book</a></td>
